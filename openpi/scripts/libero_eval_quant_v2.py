#!/usr/bin/env python3
"""
LIBERO evaluation comparing quantization methods using proper preprocessing.
"""
...
def create_model(checkpoint_dir, method='bf16'):
    """Create PI0 model with optional quantization."""
    ...
    elif method == 'w4a8_triton':
        from openpi.models_pytorch.w4a8_triton import replace_paligemma_mlp_with_w4a8_triton
        replaced = replace_paligemma_mlp_with_w4a8_triton(model, use_triton=True)
        logger.info(f"W4A8 Triton: Replaced {replaced} MLP layers (Triton FP8 quantization)")
...
def run_episode(model, tokenizer, env, task_description, initial_states, episode_idx, device,
    ...
    while t < max_steps + num_steps_wait:
        ...
        if not action_plan:
            # Prepare observation (following original script exactly)
            ...
            observation = prepare_observation(tokenizer, img, wrist_img, normalized_state, task_description, device, max_token_len)
...
def evaluate_method(method, checkpoint_dir, task_suite_name, num_tasks, num_trials, seed, resize_size, replan_steps):
    ...
    for task_id in range(min(num_tasks, task_suite.n_tasks)):
        ...
        for episode_idx in range(num_trials):
            ...
            logger.info(f"  Trial {episode_idx + 1}: {status} ({latency:.1f} ms)")
        ...
        logger.info(f"  Task success rate: {success_rate:.1%}")
...
def main():
    parser = argparse.ArgumentParser(description="LIBERO quantization comparison v2")
    ...
    parser.add_argument("--methods", type=str, nargs='+',
                       default=['bf16', 'w4a8_triton'],
                       choices=['bf16', 'w4a16', 'w4a8', 'w4a8_triton', 'w4a4'],
                       help="Methods to test")
    args = parser.parse_args()

    print("=" * 70)
    print("LIBERO Quantization Comparison v2")
    print("=" * 70)
    ...
    for method in args.methods:
        ...
        print(f"\n{method.upper()}: {result['success_rate']:.1%} ({result['total_success']}/{result['total_trials']}), "
    ...
    print(f"\n{'Method':<10} {'Success Rate':<15} {'Latency (ms)':<15} {'Hz':<10}")
    print("-" * 55)

    for method, data in all_results.items():
        print(f"{method.upper():<10} {data['success_rate']:.1%} ({data['total_success']}/{data['total_trials']}) "
              f"{data['avg_latency_ms']:<15.1f} {data['hz']:<10.2f}")

    print("-" * 55)

    # Find best method
    best_method = max(all_results.keys(), key=lambda m: all_results[m]['success_rate'])
    print(f"\nBest method: {best_method.upper()}")
    print(f"  Success Rate: {all_results[best_method]['success_rate']:.1%}")
    print(f"  Latency: {all_results[best_method]['avg_latency_ms']:.1f} ms ({all_results[best_method]['hz']:.2f} Hz)")
