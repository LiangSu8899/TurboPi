#!/usr/bin/env python3
"""LIBERO evaluation with configurable denoising steps."""

import sys
sys.path.insert(0, "/workspace/src")
sys.path.insert(0, "/workspace/packages/openpi-client/src")
sys.path.insert(0, "/workspace/third_party/libero")
...
def eval_libero(args):
    ...
    # Set denoising steps if the model supports it
    if hasattr(policy, 'model') and hasattr(policy.model, 'num_steps'):
        policy.model.num_steps = args.denoising_steps
        logging.info(f"Set denoising steps to {args.denoising_steps}")
        ...
        for episode_idx in tqdm.tqdm(range(args.num_trials), desc=f"Task {task_id}", leave=False):
            ...
            while t < max_steps + args.num_steps_wait:
                ...
                if len(action_plan) == 0:
                    ...
                    element = {
                        "observation/image": np.expand_dims(np.stack([img, img]), axis=0),
                        "observation/wrist_image": np.expand_dims(np.stack([wrist_img, wrist_img]), axis=0),
                        "observation/state": np.expand_dims(np.concatenate((
                    ...
                    result = policy.infer(element, num_steps=args.denoising_steps)
    ...
    print("\n" + "=" * 60, flush=True)
    print(f">>> Final Results ({args.task_suite_name}, {args.denoising_steps} steps): {total_successes}/{total_episodes} ({100*total_successes/total_episodes:.1f}%)", flush=True)
    print("=" * 60, flush=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_dir", default="/root/.cache/openpi/pytorch_checkpoints/pi05_libero")
    ...
    parser.add_argument("--denoising_steps", type=int, default=10, help="Number of denoising steps")
