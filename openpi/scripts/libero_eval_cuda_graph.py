#!/usr/bin/env python3
"""
LIBERO Evaluation with CUDA Graph Optimized Denoise.

Tests the ChainedDenoiseGraphs optimization on real LIBERO benchmark tasks.

Performance targets:
- Denoise: 109ms (with CUDA Graph) vs 182ms (without)
- Total pipeline: ~100ms (10 Hz)

Usage:
    docker exec turbo_pi_eval python /workspace/scripts/libero_eval_cuda_graph.py --quick
    docker exec turbo_pi_eval python /workspace/scripts/libero_eval_cuda_graph.py --task_suite_name libero_spatial --num_trials 5
...
# Skip TVM to avoid import issues
...
# Disable cuDNN for Thor SM110 compatibility
...
class CUDAGraphOptimizedPolicy:
    """
    PyTorch Policy with CUDA Graph Optimized Denoise Loop.

    Uses ChainedDenoiseGraphs for ~73ms denoise savings (40% speedup).
    """
    ...
    def _capture_cuda_graphs(self, state, prefix_kv_cache, prefix_pad_masks):
        """Capture CUDA Graphs for denoise loop."""
        from openpi.modules.graphed_denoise import ChainedDenoiseGraphs

        self._graphed_denoise = ChainedDenoiseGraphs(
            model=self.model,
            num_steps=self.num_denoising_steps,
            device=torch.device(self.device),
        )

        logger.info(f"Capturing {self.num_denoising_steps} CUDA Graphs...")
        self._graphed_denoise.capture(
            state=state,
        ...
        self._cached_prefix_len = prefix_pad_masks.shape[1]
        logger.info(f"CUDA Graphs captured (prefix_len={self._cached_prefix_len})")

    def infer(self, observation: Dict[str, Any]) -> Dict[str, np.ndarray]:
        """Run inference with CUDA Graph optimized denoise."""
        ...
        with torch.no_grad():
            ...
            torch.cuda.synchronize()
            vision_kv_time = (time.perf_counter() - vision_start) * 1000
            self.component_latencies['vision_kv'].append(vision_kv_time)
            ...
            # Check if we need to (re)capture graphs
            if not self._graph_captured or prefix_len != self._cached_prefix_len:
                self._capture_cuda_graphs(state, prefix_kv_cache, prefix_pad_masks)
            else:
                # Update KV cache AND position IDs for new observation
                # Position IDs depend on actual prefix length (sum of mask), not shape
                self._graphed_denoise.update_kv_cache(prefix_kv_cache, prefix_pad_masks)
            ...
            actions = self._graphed_denoise(noise)
    ...
    def warmup(self, num_iterations: int = 5):
        """Warmup the policy and capture CUDA graphs."""
        logger.info(f"Warming up policy ({num_iterations} iterations)...")
        ...
        for k in self.component_latencies:
            self.component_latencies[k].clear()

        logger.info("Warmup complete, CUDA Graphs captured")
...
def run_episode(env, policy, task_description, obs, args):
    """Run a single episode.

    Note: env should already be reset and initial state set before calling this.
    """
    action_plan = collections.deque()
    max_steps = MAX_STEPS_DICT.get(args.task_suite_name, 300)

    t = 0
    # Don't reset here - env is already set up by caller
...
def eval_libero(args):
    ...
    # Create policy with CUDA Graph optimization
    logger.info(f"Creating CUDA Graph Optimized Policy (denoising_steps={args.denoising_steps})")
    policy = CUDAGraphOptimizedPolicy(
        checkpoint_dir=args.checkpoint_dir,
        num_denoising_steps=args.denoising_steps,
    )

    # Warmup (captures CUDA graphs)
    policy.warmup(num_iterations=5)
    ...
    if args.quick:
        task_start = 0
        task_end = min(2, num_tasks_in_suite)  # 2 tasks for quick test
        num_trials = 3
    ...
    for task_id in tqdm.tqdm(task_range, desc="Tasks"):
        ...
        for episode_idx in tqdm.tqdm(range(num_trials), desc=f"Task {task_id}", leave=False):
            ...
            success = run_episode(env, policy, task_description, obs, args)
    ...
    # Final results
    print("\n" + "=" * 70)
    print(f"CUDA GRAPH OPTIMIZED PIPELINE EVALUATION")
    print("=" * 70)
    print(f"\nConfiguration:")
    ...
    print(f"  Hz:   {latency_stats.get('hz', 0):.2f}")
    ...
    if breakdown:
        ...
        for name in ['vision_kv', 'denoise']:
            if name in breakdown:
                b = breakdown[name]
                display_name = {'vision_kv': 'Vision+KV', 'denoise': 'Denoise CUDA'}[name]
    ...
    # Target check
    target_hz = 10.0
    achieved_hz = latency_stats.get('hz', 0)
    print(f"\n  Target: {target_hz:.1f} Hz (100 ms)")
    print(f"  Achieved: {achieved_hz:.2f} Hz ({1000/achieved_hz if achieved_hz > 0 else 0:.2f} ms)")
    if achieved_hz >= target_hz:
        print(f"  Status: ✓ TARGET ACHIEVED!")
    else:
        gap_ms = 1000/achieved_hz - 100 if achieved_hz > 0 else 0
        print(f"  Status: ✗ Need {gap_ms:.1f} ms more savings")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="LIBERO evaluation with CUDA Graph Optimized Pipeline")
    ...
    parser.add_argument("--quick", action="store_true", help="Quick mode: 2 tasks, 3 trials")
