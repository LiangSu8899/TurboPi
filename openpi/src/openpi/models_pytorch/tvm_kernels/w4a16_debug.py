...
def main():
    ...
    # Set W_packed to have value 8 (signed=0) in low nibble, 8 in high nibble
    W_packed_np[:] = 0x88  # Both nibbles = 8, signed = 0
    W_packed_tvm.copyfrom(W_packed_np)
    ...
    C_result = C_tvm.numpy()
    print(f"Expected: 0 (since signed_val = 8 - 8 = 0)")
    print(f"Actual output[0,0]: {C_result[0, 0]}")
