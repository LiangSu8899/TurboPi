#!/usr/bin/env python3
"""Quick verification of 128-bit vectorized kernel performance stability."""
...
# 5 benchmark runs
total_bytes = num_scale_blocks * N * 4 * 4 + num_scale_blocks * N * 2 + K * 2
results = []
...
for run in range(5):
    ...
    print(f"Run {run+1}: {avg_ms:.4f} ms, {bw:.1f} GB/s")

print(f"\nMean: {np.mean(results):.4f} ms")
print(f"Std:  {np.std(results):.4f} ms")
print(f"Min:  {np.min(results):.4f} ms")
print(f"Max:  {np.max(results):.4f} ms")

# Try to dump CUDA/PTX
print("\n" + "="*60)
print("Dumping CUDA source...")
print("="*60)

try:
    # Save to file for inspection
    with tvm.transform.PassContext(opt_level=3, config={"tir.emit_device_code": True}):
        lib2 = tvm.build(mod, target=target)
    ...
    try:
        ...
        for m in lib2:
            if m.type_key == "cuda":
                ...
                break
        if cuda_mod:
            src = cuda_mod.get_source()
            print("CUDA source (first 2000 chars):")
            print(src[:2000])
    except Exception as e:
        print(f"Could not get CUDA source: {e}")

except Exception as e:
    print(f"Could not dump source: {e}")

print("\n" + "="*60)
print(f"FINAL RESULT: {np.mean(results):.4f} ms (target < 0.2 ms)")
print("="*60)
if np.mean(results) < 0.2:
    print("TARGET ACHIEVED!")
else:
    print(f"Gap: {np.mean(results)/0.2:.1f}x slower")
