...
class BaseModelConfig(abc.ABC):
    ...
    def load_pytorch(self, train_config, weight_path: str):
        ...
        # Fix weight tying: embed_tokens may not be in checkpoint (LeRobot format ties it with lm_head)
        ...
        try:
            ...
            # Check if embed_tokens looks uninitialized (random init has different stats than trained weights)
            # lm_head should have been loaded from checkpoint
            if embed_tokens.shape == lm_head.shape:
                # Copy lm_head weights to embed_tokens (they should be tied)
                with torch.no_grad():
