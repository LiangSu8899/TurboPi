...
class Policy(BasePolicy):
    ...
    def infer(self, obs: dict, *, noise: np.ndarray | None = None) -> dict:  # type: ignore[misc]
        ...
        if not self._is_pytorch_model:
            ...
            # Convert inputs to PyTorch tensors and move to correct device with bfloat16 dtype for floating point
            def _to_pytorch(x):
                t = torch.from_numpy(np.array(x))
                if t.dtype in (torch.float32, torch.float64):
                    t = t.to(torch.bfloat16)
                return t.to(self._pytorch_device)[None, ...]
        ...
        if noise is not None:
            noise = torch.from_numpy(noise).to(torch.bfloat16).to(self._pytorch_device) if self._is_pytorch_model else jnp.asarray(noise)
